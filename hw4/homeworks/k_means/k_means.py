...
def calculate_centers(
    ...
    data_count = np.zeros(num_centers)
    for i in range(n):
        idx = int(classifications[i])
        data_count[idx] += 1
        new_centers[idx] += data[i]
    for c in range(num_centers):
        new_centers[c] /= data_count[c]
    return new_centers
...
def cluster_data(data: np.ndarray, centers: np.ndarray) -> np.ndarray:
    ...
    clusters = np.zeros(n)
    for i in range(n):
        #means = np.mean(np.abs(centers - data[i]), axis=1)
        distances = [np.linalg.norm(center - data[i]) for center in centers]
...
def lloyd_algorithm(
    ...
    """
    centers = prev_centers = data[:num_centers]

    errors = []
    while not errors or np.amax(abs(centers - prev_centers)) > epsilon:
        clusters = cluster_data(data, centers)
        prev_centers = centers
        centers = calculate_centers(data, clusters, num_centers=num_centers)
        errors.append(calculate_error(data, centers))
        print("error:", errors[-1])
        print("diff:", np.amax(abs(centers - prev_centers)))
